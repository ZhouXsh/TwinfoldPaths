...
出口格永不阻挡（R-05）。两角色的阻挡判定**并行**基于回合开始状态（一方停留不影响另一方候选，除非触发 P5）。判定同时输出阻挡原因（按上表优先级：边界/墙与坍塌/普通门/专属门/脉冲门/单向；P3 暂停记为独立原因），经 `MoveResult` 供表现层区分反馈（M5：单向格反馈必须与墙不同）。
...
| 子阶段        | 内容                                                                                                                                                                                                 | 规则出处 |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| D1 脆弱格坍塌 | 回合开始时所在格为脆弱格、且该角色**最终位置**（D2 后）不再等于该格、且该格尚未坍塌、且 **D2 后该格无任一角色占据**（对穿/传送落点占位不坍塌，ADR-016）→ 记入 `fragileCollapsed`；被阻挡而停留不坍塌 | M7       |
| D2 传送       | 见 §3 传送算法                                                                                                                                                                                       | M6       |
...
function resolveTurn(state, input) -> MoveResult:
  ...
  collapseFragiles(state, 起始脆弱格且最终位置已离开、且 D2 后该格无角色占据的集合)   // D1（ADR-016）
